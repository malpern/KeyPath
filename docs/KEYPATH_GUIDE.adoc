...
- Incremental local iteration (fast): `./Scripts/quick-deploy.sh` (run `./build.sh` once first)
...
----
keypath uninstall
keypath uninstall --delete-config
----
...
4. **Check logs** - View system logs: `tail -f /var/log/com.keypath.kanata.stderr.log`
...
KeyPath logs to:
- **System logs**: `/var/log/com.keypath.kanata.stdout.log` and `/var/log/com.keypath.kanata.stderr.log` (if running as LaunchDaemon)
...
1. **Backup first**: Copy `~/.config/keypath/keypath.kbd`
...
|===
| Path | Description
| `~/.config/keypath/` | Main KeyPath configuration directory (user-owned config + rule state)
| `~/.config/keypath/keypath.kbd` | Main configuration file
| `~/.config/keypath/RuleCollections.json` | Rule collections state
| `~/.config/keypath/CustomRules.json` | Custom rules
| `~/.config/keypath/AppKeymaps.json` | App-specific keymaps (if configured)
...
| `/var/log/com.keypath.kanata.stdout.log` | Kanata stdout log (if running as LaunchDaemon)
| `/var/log/com.keypath.kanata.stderr.log` | Kanata stderr/error log (if running as LaunchDaemon)
| `~/Library/Application Support/KeyPath/Favicons/` | Cached website icons (Quick Launch)
| `~/Library/Application Support/KeyPath/ActivityLog/` | Encrypted activity logs (if enabled)
| `~/Library/Logs/KeyPath/keypath-debug.log` | KeyPath application log
|===
...
KeyPath stores your Kanata config and related rule state under `~/.config/keypath/`. You can sync that directory using iCloud, Dropbox, or any file sync service.

If you also use optional features like Quick Launch or Activity Logging, those feature caches/logs live under `~/Library/Application Support/KeyPath/` and can be synced separately.
