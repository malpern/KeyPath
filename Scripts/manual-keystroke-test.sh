...
# Clear screen so the passage is front and center
clear
...
echo "║   Typos are fine — we only care about repeated characters. ║"
...
echo
echo "  Any 🔴 duplicate alerts from Kanata will appear below:"
echo
echo "  ─── live alerts ───"
echo
...
# Note: pipe through tr to ensure single-line numeric output
KEY_EVENT_COUNT=$(echo "$NEW_LOG_LINES" | grep -c "KeyInput:" 2>/dev/null || echo "0"); KEY_EVENT_COUNT=$(echo "$KEY_EVENT_COUNT" | tr -d '[:space:]')
LAYER_EVENT_COUNT=$(echo "$NEW_LOG_LINES" | grep -c "CurrentLayerName" 2>/dev/null || echo "0"); LAYER_EVENT_COUNT=$(echo "$LAYER_EVENT_COUNT" | tr -d '[:space:]')
...
# Save filtered lines to files to avoid grep -c multi-line issues
echo "$NEW_LOG_LINES" | grep "DUPLICATE DETECTION" | grep -ivE "Key '$IGNORED_KEYS_PATTERN'" > "$TEST_DIR/dup-alerts-text.txt" 2>/dev/null || true
echo "$NEW_LOG_LINES" | grep "Skipping duplicate" | grep -ivE "duplicate: $IGNORED_KEYS_PATTERN " > "$TEST_DIR/dedup-skips-text.txt" 2>/dev/null || true
echo "$NEW_LOG_LINES" | grep "DUPLICATE DETECTION" | grep -iE "Key '$IGNORED_KEYS_PATTERN'" > "$TEST_DIR/dup-alerts-ignored.txt" 2>/dev/null || true
echo "$NEW_LOG_LINES" | grep "Skipping duplicate" | grep -iE "duplicate: $IGNORED_KEYS_PATTERN " > "$TEST_DIR/dedup-skips-ignored.txt" 2>/dev/null || true
