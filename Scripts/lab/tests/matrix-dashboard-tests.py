...
class MatrixDashboardTests(unittest.TestCase):
    ...
    def test_visual_system_ties_motion_to_live_work(self) -> None:
        ...
        self.assertIn("Campaign journey", decoded)
        self.assertIn("system-ui,-apple-system,BlinkMacSystemFont", decoded)
        self.assertIn("#f8f9fb", decoded)
        self.assertIn("color-scheme:light dark", decoded)
        self.assertIn("@media (prefers-color-scheme:dark)", decoded)
        self.assertIn("--void:#0f1218", decoded)
        self.assertIn(".lab-stage-inspector[data-inspector-mode=\"inspect\"]", decoded)
        self.assertIn("@keyframes matrix-route", decoded)
        self.assertIn('class="workstream"', decoded)
        self.assertIn("@keyframes matrix-stream", decoded)
        self.assertIn("@keyframes matrix-job-packet", decoded)
        self.assertIn("function updateJobButton", decoded)
        self.assertIn("waveSignature", decoded)
        self.assertIn("previousSceneIndex", decoded)
        self.assertIn("function updateLabScene", decoded)
        self.assertIn('id="matrix-stage-inspector"', decoded)
        self.assertIn('data-stage-key="install" tabindex="0"', decoded)
        self.assertIn("function renderLabInspector", decoded)
        self.assertIn("stage.addEventListener('pointerenter'", decoded)
        self.assertIn("job.leaseId?`lease ${job.leaseId}`:null", decoded)
        self.assertIn('.lab-stage:focus-visible', decoded)
        self.assertIn('@media (hover:hover) and (pointer:fine) { #keypath-matrix .lab-stage', decoded)
        self.assertNotIn('class="lab-track"', decoded)
        self.assertNotIn('class="lab-playhead"', decoded)
        self.assertIn('lab-handoff-arrive', decoded)
        self.assertIn('data-stage-state="active"', decoded)
        self.assertIn("renderEvents((state.events||[]).slice(0,12))", decoded)
        self.assertIn("setMetric('metric-time',campaign.startedAt?fmtDuration(liveElapsed):'—',false)", decoded)
        self.assertIn("state-changed", decoded)
        self.assertIn('class="lab-theater"', decoded)
        self.assertIn('class="artifact-box"', decoded)
        self.assertIn('class="vm-window"', decoded)
        self.assertIn('class="shield-scan"', decoded)
        self.assertIn('class="evidence-page"', decoded)
        self.assertIn("function labPhase(job)", decoded)
        ...
        self.assertNotIn("@keyframes matrix-working-card", decoded)
        self.assertNotIn("@keyframes matrix-wave-breathe", decoded)
