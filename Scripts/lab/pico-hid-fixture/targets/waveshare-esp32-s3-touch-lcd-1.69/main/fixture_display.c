...
static void render(const fixture_ui_output_t *output, uint64_t now_ms) {
    lv_color_t accent = accent_for(output->scene);
    int energy = output->energy_per_mille;
    float speed = output->scene == FIXTURE_UI_RUNNING ? 0.19f : 0.055f;
#if CONFIG_KEYPATH_FIXTURE_REDUCED_MOTION
    speed = 0.0f;
#endif
    if (output->quality != FIXTURE_UI_PROTECTED) ui.phase += speed;
    ...
    lv_label_set_text_static(ui.state, scene_name(output->scene));
    ...
    lv_arc_set_value(ui.progress, output->progress_per_mille);
    ...
    pulse = 0;
#endif
    lv_obj_set_size(ui.halo_inner, 119 + pulse / 2 + energy / 90,
                    119 + pulse / 2 + energy / 90);
    lv_obj_align(ui.halo_inner, LV_ALIGN_CENTER, 0, -2);
    ...
    for (size_t index = 0; index < KEY_COUNT; ++index) {
        ...
        lv_obj_set_style_bg_opa(ui.keys[index], (lv_opa_t)opacity, 0);
    }
