...
final class PreferencesServiceTCPTests: XCTestCase {
    ...
    func testDefaultTCPConfiguration() {
        ...
        XCTAssertTrue(
            preferencesService.shouldUseTCPServer, "Should use TCP server with valid defaults"
        )
        XCTAssertEqual(
            preferencesService.tcpEndpoint, "127.0.0.1:37000",
            "TCP endpoint should be formatted correctly"
        )
    }
    ...
    func testTCPSettingsPersistence() {
        ...
        XCTAssertFalse(
            ...
            "TCP enabled setting should persist to UserDefaults"
        )
        ...
        XCTAssertEqual(
            UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 8080,
            "TCP port should persist to UserDefaults"
        )

        preferencesService.tcpServerPort = 65535
        XCTAssertEqual(
            UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 65535,
            "Maximum valid port should persist to UserDefaults"
        )
    }
    ...
    func testTCPSettingsRestoration() {
        ...
        XCTAssertFalse(
            UserDefaults.standard.bool(forKey: "KeyPath.TCP.ServerEnabled"),
            "TCP enabled setting should be stored in UserDefaults"
        )
        XCTAssertEqual(
            UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 9999,
            "TCP port setting should be stored in UserDefaults"
        )
    }
    ...
    func testPortValidation() {
        ...
        XCTAssertFalse(
            preferencesService.isValidTCPPort(1023), "Port 1023 should be invalid (below minimum)"
        )
        XCTAssertFalse(
            preferencesService.isValidTCPPort(65536), "Port 65536 should be invalid (above maximum)"
        )
        XCTAssertFalse(preferencesService.isValidTCPPort(-1), "Negative port should be invalid")
        XCTAssertFalse(preferencesService.isValidTCPPort(999_999), "Very large port should be invalid")
    }
    ...
    func testShouldUseTCPServerLogic() {
        ...
        preferencesService.tcpServerPort = 8080
        XCTAssertTrue(
            preferencesService.shouldUseTCPServer, "Should use TCP server when enabled with valid port"
        )
        ...
        XCTAssertFalse(
            ...
            "Should not use TCP server when disabled even with valid port"
        )
        ...
        XCTAssertFalse(
            preferencesService.shouldUseTCPServer,
            "Should not use TCP server when enabled with invalid port"
        )
        ...
        XCTAssertFalse(
            preferencesService.shouldUseTCPServer,
            "Should not use TCP server when disabled with invalid port"
        )
    }
    ...
    func testTCPEndpointGeneration() {
        ...
        XCTAssertEqual(
            preferencesService.tcpEndpoint, "127.0.0.1:9000",
            "TCP endpoint should be formatted correctly when enabled"
        )

        // Test different ports
        preferencesService.tcpServerPort = 37000
        XCTAssertEqual(
            preferencesService.tcpEndpoint, "127.0.0.1:37000", "TCP endpoint should reflect port changes"
        )
    ...
    func testTCPConfigDescription() {
        ...
        XCTAssertTrue(
            enabledDescription.contains("enabled"), "Description should indicate TCP is enabled"
        )
        ...
        XCTAssertTrue(
            disabledDescription.contains("disabled"), "Description should indicate TCP is disabled"
        )
    }
    ...
    func testResetTCPSettings() {
        ...
        XCTAssertEqual(
            preferencesService.tcpServerPort, 37000, "TCP port should be default after reset"
        )

        // Verify persistence of reset values
        XCTAssertTrue(
            UserDefaults.standard.bool(forKey: "KeyPath.TCP.ServerEnabled"),
            "Reset TCP enabled should persist"
        )
        XCTAssertEqual(
            UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 37000,
            "Reset TCP port should persist"
        )
    }
        ...
        for i in 0 ..< 10 {
            Task {
                await MainActor.run {
                    ...
                    XCTAssertEqual(
                        preferencesService.tcpServerPort, port,
                        "Port should be set correctly in concurrent access"
                    )
                    XCTAssertEqual(
                        preferencesService.tcpServerEnabled, i % 2 == 0,
                        "Enabled state should be set correctly in concurrent access"
                    )
                }
                expectation.fulfill()
            }
    ...
    func testEdgeCasePortValues() {
        ...
        XCTAssertFalse(
            preferencesService.isValidTCPPort(1023), "Port 1023 should be invalid (just below minimum)"
        )
        XCTAssertTrue(preferencesService.isValidTCPPort(1024), "Port 1024 should be valid (minimum)")
        XCTAssertTrue(preferencesService.isValidTCPPort(65535), "Port 65535 should be valid (maximum)")
        XCTAssertFalse(
            preferencesService.isValidTCPPort(65536), "Port 65536 should be invalid (just above maximum)"
        )
    ...
    func testUserDefaultsKeyConsistency() {
        ...
        XCTAssertTrue(
            UserDefaults.standard.bool(forKey: "KeyPath.TCP.ServerEnabled"),
            "TCP enabled key should match"
        )
        XCTAssertEqual(
            UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 12345,
            "TCP port key should match"
        )
    }
