...
class SystemStateDetectorTests: XCTestCase {
    ...
    func testDetectsActualSystemState() async {
        ...
        XCTAssertNotEqual(result.state, .initializing, "Should complete detection and return actual state")
        
        // Should have detection timestamp
        XCTAssertLessThan(Date().timeIntervalSince(result.detectionTimestamp), 5.0, "Detection should be recent")
    ...
    func testPermissionCheckingWithRealSystem() async {
        ...
        XCTAssertFalse(result.granted.isEmpty || result.missing.isEmpty, 
        ...
        XCTAssertTrue(grantedSet.isDisjoint(with: missingSet), 
        ...
        XCTAssertEqual(checkedPermissions, allPermissions, 
    ...
    func testComponentCheckingWithRealSystem() async {
        ...
        XCTAssertTrue(installedSet.isDisjoint(with: missingSet), 
        ...
        XCTAssertEqual(checkedComponents, allComponents, 
    ...
    func testAutoFixActionsBasedOnRealState() async {
        ...
        for action in result.autoFixActions {
            ...
            case .startKarabinerDaemon:
                XCTAssertEqual(result.state, .daemonNotRunning, "Should only suggest daemon start if not running")
    ...
    func testNavigationLogicWithRealState() async {
        ...
        case .missingPermissions(let missing):
            if missing.contains(.keyPathInputMonitoring) || missing.contains(.kanataInputMonitoring) {
                ...
                XCTAssertEqual(currentPage, .accessibility, "Should show accessibility if only that missing")
            }
