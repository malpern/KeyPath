...
final class WizardKanataServicePageTests: XCTestCase {
    ...
    func testErrorDetectionFromLogFile() {
        ...
        let hasPermissionError = testLogEntries.contains { $0.contains("ERROR") && $0.contains("Permission denied") }
    ...
    func testServicePageAppearsWhenServiceNotRunning() {
        ...
        let pageForNotRunning = navigationEngine.determineCurrentPage(for: serviceNotRunningState, issues: [])
        let pageForReady = navigationEngine.determineCurrentPage(for: readyState, issues: [])
        
        XCTAssertEqual(pageForNotRunning, .service, "Should route to service page when service not running")
        XCTAssertEqual(pageForReady, .service, "Should route to service page when ready to start service")
    ...
    func testServicePageInWizardFlow() {
        ...
        guard let serviceIndex = pageOrder.firstIndex(of: .service),
              ...
              let summaryIndex = pageOrder.firstIndex(of: .summary) else {
    ...
    func testServiceControlButtonStates() {
        ...
        if isRunning {
            XCTAssertFalse(startButtonShouldBeEnabled, "Start button should be disabled when service is running")
            XCTAssertTrue(stopButtonShouldBeEnabled, "Stop button should be enabled when service is running")
        } else {
            XCTAssertTrue(startButtonShouldBeEnabled, "Start button should be enabled when service is stopped")
            XCTAssertFalse(stopButtonShouldBeEnabled, "Stop button should be disabled when service is stopped")
        }
        
        XCTAssertTrue(restartButtonShouldBeEnabled, "Restart button should be enabled when not performing action")
