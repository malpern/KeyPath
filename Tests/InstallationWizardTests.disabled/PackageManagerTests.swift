...
final class PackageManagerTests: XCTestCase {
    ...
    func testDetectKanataInstallation_ReturnsValidInfo() {
        ...
        if kanataInfo.isInstalled {
            ...
            XCTAssertNotEqual(kanataInfo.installationType, .notInstalled, "Installation type should not be notInstalled when installed")
            ...
            XCTAssertEqual(kanataInfo.installationType, .notInstalled, "Installation type should be notInstalled when not installed")
        }
    ...
    func testGetPackageManagerInfo_ReturnsValidInfo() {
        ...
        XCTAssertTrue(info.kanataInstallation.isInstalled == true || info.kanataInstallation.isInstalled == false)
        ...
        if !info.homebrewAvailable {
            ...
            XCTAssertTrue(info.supportedPackageManagers.isEmpty, "No supported package managers should be available when Homebrew is not available")
        }
    }
...
class MockPackageManager: PackageManager {
    ...
    init(homebrewAvailable: Bool = true, kanataInstalled: Bool = false, kanataPath: String? = nil, installationType: KanataInstallationType = .notInstalled) {
    ...
    override func checkHomebrewInstallation() -> Bool {
        return mockHomebrewAvailable
    }
    
    override func getHomebrewPath() -> String? {
        return mockHomebrewAvailable ? "/opt/homebrew/bin/brew" : nil
    }
    
    override func getHomebrewBinPath() -> String? {
        return mockHomebrewAvailable ? "/opt/homebrew/bin" : nil
    }
...
final class MockPackageManagerTests: XCTestCase {
    ...
    func testEdgeCase_HomebrewAvailable_KanataNotInstalled() {
        ...
        XCTAssertGreaterThan(kanataRecommendations.count, 0, "Should recommend Kanata when not installed")
        ...
        XCTAssertGreaterThan(homebrewRecommendations.count, 0, "Should recommend Homebrew method when available")
    }
    ...
    func testEdgeCase_NoHomebrew_NoKanata() {
        ...
        XCTAssertGreaterThan(manualRecommendations.count, 0, "Should recommend manual installation when no package manager")
    }
    ...
    func testEdgeCase_CargoInstallation() {
        ...
        let recommendations = mockManager.getInstallationRecommendations()
        let karabinerRecommendations = recommendations.filter { $0.package.name == "Karabiner-Elements" }
        XCTAssertGreaterThan(karabinerRecommendations.count, 0, "Should always recommend Karabiner-Elements")
    }
}