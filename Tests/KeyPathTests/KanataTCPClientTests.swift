...
final class KanataTCPClientTests: XCTestCase {
    ...
    func testConfigValidationFailure() async {
        ...
        case .failure(let errors):
            ...
            XCTAssertTrue(errors[0].message.contains("invalid-option"), "First error should mention invalid option")
            XCTAssertEqual(errors[1].line, 4, "Second error should be on line 4")
            XCTAssertTrue(errors[1].message.contains("parenthesis"), "Second error should mention parenthesis")
        case .networkError(let message):
            XCTFail("Should not have network error: \(message)")
        }
    }
    ...
    func testConfigValidationTimeout() async {
        ...
        case .networkError(let message):
            XCTAssertTrue(message.contains("timeout") || message.contains("timed out"), "Error message should indicate timeout: \(message)")
    ...
    func testConcurrentValidationRequests() async {
        ...
        let tasks = configs.map { config in
            Task {
                return await client.validateConfig(config)
            }
        }
