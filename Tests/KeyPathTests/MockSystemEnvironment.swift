...
class MockSystemEnvironment {
    ...
    func setupPartialInstallation() {
        ...
        addMockFile(path: "/usr/local/bin/kanata-cmd", 
    ...
    func setupCompleteInstallation() {
        ...
        addMockFile(path: "/usr/local/bin/kanata-cmd", 
                   ...
                   permissions: "755", 
                   owner: "root")
        addMockFile(path: "/Library/LaunchDaemons/com.keypath.kanata.plist", 
                   content: mockLaunchDaemonPlist(), 
                   permissions: "644", 
                   owner: "root")
        addMockFile(path: "/Library/Application Support/org.pqrs/Karabiner-DriverKit-VirtualHIDDevice", 
                   content: "mock-driver", 
                   permissions: "755", 
                   owner: "root")
...
class MockKanataManager: ObservableObject {
    ...
    func isKarabinerDriverInstalled() -> Bool {
        return mockEnvironment.fileExists(atPath: "/Library/Application Support/org.pqrs/Karabiner-DriverKit-VirtualHIDDevice")
    }
    ...
    func startKanata() async {
        ...
        let result = mockEnvironment.mockLaunchctlResult(command: ["kickstart", "system/com.keypath.kanata"])
    ...
    func saveConfiguration(input: String, output: String) async throws {
        ...
        guard isCompletelyInstalled() else {
            throw NSError(domain: "MockError", code: 1, userInfo: [NSLocalizedDescriptionKey: "LaunchDaemon missing. Please run: sudo ./install-system.sh"])
        }
        
        // Simulate config validation
        if input.isEmpty || output.isEmpty {
            throw NSError(domain: "MockError", code: 2, userInfo: [NSLocalizedDescriptionKey: "Invalid configuration"])
        }
