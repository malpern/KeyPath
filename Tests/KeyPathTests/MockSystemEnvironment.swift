...
class MockSystemEnvironment {
    ...
    func fileExists(atPath path: String) -> Bool {
        mockFileSystem[path]?.exists ?? false
    }
...
class MockEnvironmentKanataManager: ObservableObject {
    ...
    func isServiceInstalled() -> Bool {
        mockEnvironment.fileExists(atPath: "/Library/LaunchDaemons/com.keypath.kanata.plist")
    }
    ...
    func isCompletelyInstalled() -> Bool {
        isInstalled() && isServiceInstalled() && isKarabinerDriverInstalled()
    }
            ...
            throw NSError(
                ...
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "LaunchDaemon missing. Please run: sudo ./install-system.sh install"
                ]
            )
        }
