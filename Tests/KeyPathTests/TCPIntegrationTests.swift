...
final class TCPIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        tempDirectory = FileManager.default.temporaryDirectory.appendingPathComponent("tcp-integration-tests-\(UUID().uuidString)")
    ...
    func testCompleteConfigurationWorkflow() async throws {
        ...
        XCTAssertTrue(preferencesService.shouldUseTCPServer, "Preferences should indicate TCP should be used")
        ...
        XCTAssertTrue(arguments.contains(String(preferencesService.tcpServerPort)), "Plist should contain correct port number")
    }
    ...
    func testWorkflowWithTCPDisabled() async throws {
        ...
        XCTAssertFalse(arguments.contains("--tcp-port"), "LaunchDaemon plist should not include TCP port when disabled")
    }
    ...
    func testWorkflowWithInvalidPort() throws {
        ...
        XCTAssertFalse(arguments.contains("--tcp-port"), "LaunchDaemon plist should not include invalid TCP port")
    }
            ...
            Task {
                await MainActor.run {
                    ...
                    if preferencesService.tcpServerEnabled && preferencesService.isValidTCPPort(port) {
                        XCTAssertTrue(preferencesService.shouldUseTCPServer, "Should use TCP with valid settings in task \(i)")
                    }
                }
                expectation.fulfill()
            }
    ...
    func testInvalidConfigRecovery() async throws {
        ...
        await mockTCPServer.setValidationResponse(success: false, errors: [
