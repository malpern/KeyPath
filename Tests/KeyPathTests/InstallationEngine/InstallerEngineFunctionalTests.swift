...
final class InstallerEngineFunctionalTests: XCTestCase {
  ...
  func testReRunningInstallerFailsWhenLaunchDaemonDirectoryReadOnly() async throws {
    ...
    try FileManager.default.setAttributes(
      [.posixPermissions: 0o555], ofItemAtPath: launchDaemonsURL.path)
    defer {
      try? FileManager.default.setAttributes(
        [.posixPermissions: 0o755], ofItemAtPath: launchDaemonsURL.path)
    }
  ...
  func testInstallerLogsPermissionDeniedWhenConfigDirectoryCannotBeCreated() async throws {
    ...
    defer {
      try? FileManager.default.setAttributes(
        [.posixPermissions: 0o755], ofItemAtPath: configDir.path)
    }
  ...
  func testCreateAllServicesLogsErrorWhenUserConfigDirectoryBlocked() async throws {
    ...
    try? FileManager.default.createDirectory(
      at: homeURL.appendingPathComponent(".config", isDirectory: true),
    ...
    let contents = waitForLogMessage(
      "Failed to create default user config", logURL: logPath, timeout: 3.0)
  ...
  func testBundledKanataMissingProducesLogWarning() async throws {
    ...
    defer { AppLogger.shared.overrideLogDirectory(previousLogDir) }
    try? FileManager.default.createDirectory(
      at: URL(fileURLWithPath: logDir.path), withIntermediateDirectories: true)
  ...
  func testSMAppServicePendingApprovalTriggersNotification() async throws {
    ...
    let result = try await PrivilegedOperationsCoordinator.shared.installServicesIfUninstalled(
      context: "pending-flow")
    XCTAssertFalse(result)
  ...
  func testInstallerFailureWhenLaunchDaemonDirectoryIsInvalid() async throws {
    ...
    defer {
      ...
      try? FileManager.default.createDirectory(
        at: launchDaemonsURL, withIntermediateDirectories: true)
    }
  ...
  func testInstallerFailureWhenLaunchDaemonsDirectoryIsReadOnly() async throws {
    ...
    XCTAssertTrue(
      contents?.contains("Failed to install plists") == true, "Log should record permission failure"
    )
  }
  ...
  func testInstallOnlySkipsKanataWhenSMAppServiceActive() async throws {
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: plistPath(for: "com.keypath.karabiner-vhiddaemon")))
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: plistPath(for: "com.keypath.karabiner-vhidmanager")))
  }
  ...
  func testKanataPlistMigrationExpandsTildePath() async throws {
    ...
    let logContents = waitForLogMessage(
      "Rewriting Kanata plist config path", logURL: logPath, timeout: 2.0)
