...
final class LaunchDaemonInstallerTests: XCTestCase {
  ...
  func testAuthServicesInstallationRespectsServiceDependencyOrder() throws {
    ...
    guard
      let methodStart = sourceCode.range(of: "executeConsolidatedInstallationWithAuthServices")?
        .lowerBound
    else {
    ...
    guard let scriptStart = methodSection.range(of: "#!/bin/bash")?.lowerBound,
      let scriptEnd = methodSection.range(
        of: "\"\"\"", range: scriptStart..<methodSection.endIndex)?.lowerBound
    else {
    ...
    XCTAssertEqual(
      bootstrapOrder, correctServiceOrder,
    ...
    XCTAssertEqual(
      kickstartOrder, correctServiceOrder,
  ...
  func testImprovedOsascriptInstallationRespectsServiceDependencyOrder() throws {
    ...
    guard
      let methodStart = sourceCode.range(of: "executeConsolidatedInstallationImproved")?.lowerBound
    else {
    ...
    guard let commandStart = methodSection.range(of: "set -ex")?.lowerBound,
      let commandEnd = methodSection.range(of: "echo \"Installation completed successfully\"")?
        .upperBound
    else {
  ...
  func testInlineScriptRespectsServiceDependencyOrder() throws {
    ...
    if foundInlineBootstrap, inlineBootstrapOrder.count == 3 {
      XCTAssertEqual(
        inlineBootstrapOrder, correctServiceOrder,
    ...
    for line in lines {
      if line.contains("launchctl bootstrap system") {
        if let serviceName = extractServiceName(from: line) {
          ...
          if serviceName == "com.keypath.kanata" {
            if currentBootstrapSequence.count == 1 {
              XCTFail(
                "Found Kanata being bootstrapped without VirtualHID services loaded first at line: \(line)"
              )
            }
          }
        }
        ...
        if currentBootstrapSequence.count >= 3 {
          ...
          XCTAssertEqual(
            currentBootstrapSequence, correctServiceOrder,
  ...
  func testDependencyOrderIsDocumented() throws {
    ...
    let hasDepComment =
      sourceCode.contains("DEPENDENCIES FIRST")
