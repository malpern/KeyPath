...
final class EngineReloadSingleFlightTests: XCTestCase {
    ...
    func testRunCoalescesConcurrentCalls() async {
        ...
        // Give the first task a chance to start and set inFlight before opening the gate.
        ...
        let startedValue = await started.get()
        XCTAssertEqual(startedValue, 1, "Only one operation should execute")
