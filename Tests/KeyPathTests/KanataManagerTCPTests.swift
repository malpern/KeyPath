...
final class KanataManagerTCPTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        tempConfigDirectory = FileManager.default.temporaryDirectory.appendingPathComponent("keypath-tcp-tests-\(UUID().uuidString)")
    ...
    func testTCPValidationFailureWithErrors() async throws {
        ...
        XCTAssertTrue(result.errors[0].contains("invalid-option"), "Should include specific error messages")
        XCTAssertTrue(result.errors[1].contains("parenthesis"), "Should include specific error messages")
    }
    ...
    func testBuildKanataArgumentsWithTCPEnabled() {
        ...
        XCTAssertTrue(preferencesService.shouldUseTCPServer, "Should use TCP server with valid settings")
        XCTAssertEqual(preferencesService.tcpEndpoint, "127.0.0.1:37000", "TCP endpoint should be correct")
    }
