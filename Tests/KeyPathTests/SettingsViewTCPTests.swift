...
final class SettingsViewTCPTests: XCTestCase {
    ...
    private func findAvailablePort() throws -> Int {
        ...
        if getsockname(socket, boundAddrPtr, &addrLen) < 0 {
            throw NSError(domain: "TestSetup", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to get socket name"])
        }
    ...
    func testPortDisplayAndValidation() {
        ...
        XCTAssertEqual(preferencesService.tcpServerPort, preferencesService.tcpServerPort, "Port should be correctly stored")
        ...
        for port in invalidPorts {
            ...
            XCTAssertFalse(preferencesService.shouldUseTCPServer, "Should not use TCP with invalid port \(port)")
        }
    }
    ...
    func testPortChangeValidation() {
        ...
        for (port, shouldBeValid) in testPorts {
            ...
            XCTAssertEqual(isValid, shouldBeValid, "Port \(port) validation should return \(shouldBeValid)")
            
            if shouldBeValid && preferencesService.tcpServerEnabled {
                XCTAssertTrue(preferencesService.shouldUseTCPServer, "Should use TCP with valid port \(port)")
                XCTAssertNotNil(preferencesService.tcpEndpoint, "TCP endpoint should be available for valid port \(port)")
            }
        }
    }
    ...
    func testPortChangeUserFlow() {
        ...
        XCTAssertNotEqual(preferencesService.tcpServerPort, originalPort, "Port should be different from original")
        
        // Verify persistence
        XCTAssertEqual(UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), newPort, "Port change should persist to UserDefaults")
    }
    ...
    func testInvalidPortUserInput() {
        ...
        for invalidPort in invalidPorts {
            ...
            XCTAssertFalse(preferencesService.shouldUseTCPServer, "Should not use TCP with invalid port \(invalidPort)")
            
            // But the invalid value should still be stored (for user to see and correct)
            XCTAssertEqual(preferencesService.tcpServerPort, invalidPort, "Invalid port should be stored for user feedback")
        }
    }
    ...
    func testTCPConfigurationPersistence() {
        ...
        // Verify persistence
        XCTAssertFalse(UserDefaults.standard.bool(forKey: "KeyPath.TCP.ServerEnabled"), "TCP enabled state should persist")
        XCTAssertEqual(UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 9999, "TCP port should persist")
        ...
        XCTAssertTrue(UserDefaults.standard.bool(forKey: "KeyPath.TCP.ServerEnabled"), "New TCP enabled state should persist")
        XCTAssertEqual(UserDefaults.standard.integer(forKey: "KeyPath.TCP.ServerPort"), 37000, "New TCP port should persist")
    }
    ...
    func testPortValidationErrorHandling() {
        ...
        for (port, expectedValid) in boundaryTests {
            let isValid = preferencesService.isValidTCPPort(port)
            XCTAssertEqual(isValid, expectedValid, "Port \(port) should be \(expectedValid ? "valid" : "invalid")")
        }
    }
    ...
    func testSettingsViewPreferencesIntegration() {
        ...
        XCTAssertEqual(preferencesService.tcpServerPort, 9999, "SettingsView should observe port changes")
        XCTAssertEqual(preferencesService.tcpServerEnabled, !originalEnabled, "SettingsView should observe enabled state changes")
    ...
    func testTypicalUserConfigurationFlow() async throws {
        ...
        XCTAssertEqual(preferencesService.tcpServerPort, preferencesService.tcpServerPort, "Should show current port")
    ...
    func testAdvancedUserConfigurationFlow() {
        ...
        for port in testPorts {
            ...
            XCTAssertEqual(preferencesService.tcpEndpoint, expectedEndpoint, "TCP endpoint should be correctly formatted for port \(port)")
        }
    ...
    func testSettingsViewEdgeCases() {
        ...
        XCTAssertFalse(preferencesService.isValidTCPPort(Int.max), "Should handle extremely large port numbers")
        
        preferencesService.tcpServerPort = Int.min
        XCTAssertFalse(preferencesService.isValidTCPPort(Int.min), "Should handle negative port numbers")
        ...
        XCTAssert(preferencesService.tcpServerEnabled == true || preferencesService.tcpServerEnabled == false, "Rapid toggling should maintain consistent state")
    }
