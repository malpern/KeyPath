...
struct BehaviorRenderingGoldenTests {
    ...
    func dualRoleBasic() {
        let mapping = KeyMapping(
            ...
            behavior: .dualRole(DualRoleBehavior(tapAction: "a", holdAction: "lctl"))
        )
    ...
    func dualRoleCustomTimeouts() {
        let mapping = KeyMapping(
            input: "a",
            action: .keystroke(key: "a"),
            behavior: .dualRole(DualRoleBehavior(tapAction: "a", holdAction: "lsft", tapTimeout: 150, holdTimeout: 300))
        )
    ...
    func dualRoleHyperHold() {
        let mapping = KeyMapping(
            ...
            behavior: .dualRole(DualRoleBehavior(tapAction: "esc", holdAction: "hyper"))
        )
    ...
    func dualRoleMehHold() {
        let mapping = KeyMapping(
            input: "caps",
            action: .keystroke(key: "caps"),
            behavior: .dualRole(DualRoleBehavior(tapAction: "esc", holdAction: "meh"))
        )
    ...
    func dualRoleMultiKeyHold() {
        let mapping = KeyMapping(
            ...
            behavior: .dualRole(DualRoleBehavior(tapAction: "z", holdAction: "lctl lalt"))
        )
    ...
    func dualRoleSExpressionHold() {
        let mapping = KeyMapping(
            ...
            behavior: .dualRole(DualRoleBehavior(tapAction: "spc", holdAction: "(layer-while-held nav)"))
        )
        ...
        let mapping = KeyMapping(
            ...
            behavior: .tapOrTapDance(.tapDance(TapDanceBehavior(
                ...
                steps: [
                    ...
                    TapDanceStep(label: "Double", action: "lctl"),
                ]
            )))
        )
    ...
    func tapDanceCustomWindow() {
        let mapping = KeyMapping(
            ...
            behavior: .tapOrTapDance(.tapDance(TapDanceBehavior(
                windowMs: 300,
                steps: [
                    TapDanceStep(label: "1", action: "a"),
                    TapDanceStep(label: "2", action: "b"),
                    TapDanceStep(label: "3", action: "c"),
                ]
            )))
        )
    ...
    func chordAlias() {
        let mapping = KeyMapping(
            ...
            behavior: .chord(ChordBehavior(keys: ["j", "k"], output: "esc", timeout: 200))
        )
    ...
    func chordDefinition() {
        let chord = ChordBehavior(keys: ["j", "k"], output: "esc", timeout: 200)
    ...
    func chordDefinitionMultiKey() {
        let chord = ChordBehavior(keys: ["a", "s"], output: "lctl lalt", timeout: 150)
        let result = KanataBehaviorRenderer.renderChordDefinition(chord)
