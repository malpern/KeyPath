...
final class AuthorizationServicesSmokeTests: XCTestCase {
  ...
  override func setUp() async throws {
    ...
    try FileManager.default.setAttributes(
      [.posixPermissions: 0o755], ofItemAtPath: bundledKanataURL.path)
  ...
  func testAuthorizationServicesInstallationCreatesPlistsAndBootstraps() async throws {
    ...
    XCTAssertTrue(
      logContents.contains("bootstrap system \(plistPath(for: "com.keypath.karabiner-vhiddaemon"))")
    )
    XCTAssertTrue(
      logContents.contains(
        "bootstrap system \(plistPath(for: "com.keypath.karabiner-vhidmanager"))"))
