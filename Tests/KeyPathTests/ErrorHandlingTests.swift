...
final class ErrorHandlingTests: XCTestCase {
  ...
  func testExtremeKeyCodeHandling() throws {
    ...
    for keyCode in extremeKeyCodes {
      ...
      XCTAssertTrue(
        result.hasPrefix("key") || result.count == 1
          || ["return", "space", "tab", "escape", "delete", "caps"].contains(result),
  ...
  func testConfigGenerationWithInvalidInputOutput() throws {
    ...
    for (input, output) in problematicCombos {
      ...
      XCTAssertTrue(
        config.contains("(deflayer"), "Should contain deflayer even with problematic input")
      ...
      XCTAssertEqual(
        ...
        "Should have balanced parentheses with input: '\(input.prefix(20))' output: '\(output.prefix(20))'"
      )
    }
  }
  ...
  func testConfigWithSpecialKanataCharacters() throws {
    ...
    for specialChar in specialKanataChars {
      ...
      XCTAssertEqual(
        openParens, closeParens,
        "Should have balanced parentheses with special char: \(specialChar)")
    }
  }
  ...
  func testConfigWithNonASCIICharacters() throws {
    ...
    for char in internationalChars {
      ...
      XCTAssertTrue(
        inputConfig.contains("(defcfg"), "Should handle international input char: \(char)")
      XCTAssertTrue(
        outputConfig.contains("(defcfg"), "Should handle international output char: \(char)")
      ...
      XCTAssertEqual(
        inputOpenParens, inputCloseParens, "Input config should be balanced with char: \(char)")
      ...
      XCTAssertEqual(
        outputOpenParens, outputCloseParens, "Output config should be balanced with char: \(char)")
    }
  }
    ...
    do {
      ...
      XCTAssertTrue(
        errorDesc.contains("kanata") || errorDesc.contains("config")
          || errorDesc.contains("permission") || errorDesc.contains("install")
          || errorDesc.contains("directory") || errorDesc.contains("file"),
  ...
  func testMemoryPressureErrorHandling() throws {
    ...
    for iteration in 0..<100 {
      ...
      XCTAssertTrue(
        config.contains("(defcfg"),
        "Should handle large inputs under memory pressure: iteration \(iteration)")
    }
  }
  ...
  func testInvalidPathHandling() throws {
    ...
    XCTAssertTrue(
      inLibrary || inDotConfig, "Config path should be in Library/KeyPath or ~/.config/keypath")
  ...
  func testServiceInstallationErrorHandling() throws {
    ...
    XCTAssertTrue(
      isServiceInstalled == true || isServiceInstalled == false,
      "isServiceInstalled should return boolean")
    XCTAssertTrue(
      isCompletelyInstalled == true || isCompletelyInstalled == false,
      "isCompletelyInstalled should return boolean")
  ...
  func testConfigurationIntegrity() throws {
    ...
    for input in randomInputs {
      ...
      XCTAssertTrue(
        config.contains("process-unmapped-keys yes"),
        "Random config should pass through unmapped keys: \(input)")
