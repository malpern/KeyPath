...
final class KeyboardCaptureListenOnlyTests: XCTestCase {
  ...
  func testRawModeWhenKanataNotRunning() {
    ...
    XCTAssertTrue(
      shouldSuppress, "Should suppress events in raw capture mode when Kanata is not running")
  ...
  func testRawModeWhenFeatureFlagDisabled() {
    ...
    XCTAssertTrue(
      capture.suppressEvents, "Flag off forces raw/suppress mode even if Kanata is running")
