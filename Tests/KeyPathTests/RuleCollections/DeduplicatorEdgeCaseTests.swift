...
final class DeduplicatorEdgeCaseTests: XCTestCase {
    ...
    func testFirstCollectionWins_SameKeyOnSameLayer() {
        ...
        XCTAssertEqual(aMappings.first?.output, "b", "First collection should win")
    }
    ...
    func testDisabledCollection_DoesNotClaimKeys() {
        ...
        XCTAssertEqual(aMappings.first?.output, "c", "Enabled collection should win over disabled")
    }
    ...
    func testThreeCollections_FirstStillWins() {
        let c1 = makeCollection(name: "First", mappings: [KeyMapping(input: "a", output: "1")])
        let c2 = makeCollection(name: "Second", mappings: [KeyMapping(input: "a", output: "2")])
        let c3 = makeCollection(name: "Third", mappings: [KeyMapping(input: "a", output: "3")])
        ...
        XCTAssertEqual(aMappings.first?.output, "1", "First collection should still win with 3 conflicts")
    }
    ...
    func testPartialOverlap_NonConflictingKeysPreserved() {
        ...
        XCTAssertTrue(allMappings.contains { $0.input == "s" && $0.output == "d" },
                       "Non-conflicting key 's' from first collection should survive")
        XCTAssertTrue(allMappings.contains { $0.input == "f" && $0.output == "g" },
