...
final class LaunchDaemonInstallerTCPTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        tempDirectory = FileManager.default.temporaryDirectory.appendingPathComponent("launchdaemon-tcp-tests-\(UUID().uuidString)")
    ...
    func testBuildKanataArgumentsWithTCPEnabled() {
        ...
        XCTAssertTrue(plistContent.contains("--tcp-port"), "LaunchDaemon plist should include --tcp-port argument when TCP is enabled")
        XCTAssertTrue(plistContent.contains("37000"), "LaunchDaemon plist should include the configured TCP port number")
    }
    ...
    func testBuildKanataArgumentsWithTCPDisabled() {
        ...
        XCTAssertFalse(plistContent.contains("--tcp-port"), "LaunchDaemon plist should not include --tcp-port argument when TCP is disabled")
    }
    ...
    func testBuildKanataArgumentsWithValidTCPPorts() {
        ...
        for port in validPorts {
            ...
            XCTAssertTrue(plistContent.contains("--tcp-port"), "Should include --tcp-port for valid port \(port)")
            XCTAssertTrue(plistContent.contains(String(port)), "Should include port number \(port) in plist")
        }
    }
    ...
    func testBuildKanataArgumentsWithInvalidTCPPorts() {
        ...
        for port in invalidPorts {
            ...
            XCTAssertFalse(plistContent.contains("--tcp-port"), "Should not include --tcp-port for invalid port \(port)")
        }
    }
    ...
    func testBuildKanataArgumentsWithCustomTCPPort() {
        ...
        for port in customPorts {
            ...
            XCTAssertTrue(plistContent.contains("--tcp-port"), "Should include --tcp-port argument for custom port \(port)")
            XCTAssertTrue(plistContent.contains(String(port)), "Should include custom port number \(port)")
            ...
            if let tcpPortRange = plistContent.range(of: "--tcp-port") {
                ...
                XCTAssertTrue(afterTCPPort.contains(String(port)), "Port number should appear after --tcp-port flag")
            }
        }
    }
    ...
    func testPlistGenerationWithTCPConfiguration() throws {
        ...
        XCTAssertTrue(programArguments.contains("--tcp-port"), "ProgramArguments should contain --tcp-port")
        
        if let tcpPortIndex = programArguments.firstIndex(of: "--tcp-port") {
            XCTAssertLessThan(tcpPortIndex + 1, programArguments.count, "Port number should follow --tcp-port")
    ...
    func testPlistGenerationWithoutTCPConfiguration() throws {
        ...
        XCTAssertFalse(programArguments.contains("--tcp-port"), "ProgramArguments should not contain --tcp-port when TCP is disabled")
    }
    ...
    func testPlistArgumentsOrder() throws {
        ...
        XCTAssertTrue(programArguments.first?.contains("kanata") == true, "First argument should be kanata binary path")
        
        // Verify TCP arguments appear in correct order
        if let tcpPortIndex = programArguments.firstIndex(of: "--tcp-port") {
            XCTAssertLessThan(tcpPortIndex + 1, programArguments.count, "Port number should follow --tcp-port")
            XCTAssertEqual(programArguments[tcpPortIndex + 1], "8080", "Port number should immediately follow --tcp-port")
    ...
    func testPlistGenerationWithEdgeCasePorts() throws {
        ...
        for (port, shouldInclude) in edgeCasePorts {
            ...
            if shouldInclude {
                XCTAssertTrue(programArguments.contains("--tcp-port"), "Should include --tcp-port for valid port \(port)")
                XCTAssertTrue(programArguments.contains(String(port)), "Should include port number \(port)")
            } else {
                XCTAssertFalse(programArguments.contains("--tcp-port"), "Should not include --tcp-port for invalid port \(port)")
            }
        }
    }
    ...
    func testPlistGenerationConsistency() throws {
        ...
        XCTAssertEqual(plistData1, plistData2, "Multiple plist generations should produce identical results")
    }
    ...
    func testPlistGenerationWithTCPStateChanges() throws {
        ...
        XCTAssertFalse(programArguments.contains("--tcp-port"), "Should not include TCP args when disabled")
        ...
        XCTAssertTrue(programArguments.contains("--tcp-port"), "Should include TCP args when re-enabled")
    ...
    func testPlistWriteAndRead() throws {
        ...
        let plist = try PropertyListSerialization.propertyList(from: readData, format: nil) as! [String: Any]
        ...
        XCTAssertTrue(programArguments.contains("--tcp-port"), "Written plist should contain TCP arguments")
    ...
    func testPlistValidityAndStructure() throws {
        ...
        XCTAssertTrue(programArguments.first?.contains("kanata") == true, "First argument should be kanata binary")
