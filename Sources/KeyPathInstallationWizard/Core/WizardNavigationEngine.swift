...
@MainActor
public final class WizardNavigationEngine: WizardNavigating {
    public init() {}
    ...
    public func resetNavigationState() {
    ...
    public func firstPageNeedingAttention(for state: WizardSystemState, issues: [WizardIssue]) async -> WizardPage {
    ...
    public func determineCurrentPage(for state: WizardSystemState, issues: [WizardIssue]) async -> WizardPage {
        ...
        if !hasShownFullDiskAccessPage {
            if WizardDependencies.fullDiskAccessChecker?.hasFullDiskAccess() ?? false {
    ...
    public func canNavigate(from _: WizardPage, to _: WizardPage, given _: WizardSystemState) -> Bool {
    ...
    public func createNavigationState(
    ...
    public func getPageOrder() -> [WizardPage] {
    ...
    public func pageIndex(_ page: WizardPage) -> Int {
    ...
    public func pageHasRelevantIssues(_ page: WizardPage, issues: [WizardIssue], state: WizardSystemState) -> Bool {
        switch page {
        case .summary:
    ...
    public func isBlockingPage(_ page: WizardPage) async -> Bool {
        switch page {
        case .conflicts:
        ...
        case .helper:
            ...
            let needsApproval = WizardDependencies.helperManager?.helperNeedsLoginItemsApproval() ?? false
            let notInstalled = await !(WizardDependencies.helperManager?.isHelperInstalled() ?? false)
    ...
    public func shouldShowNextButton(
    ...
    public func shouldShowPreviousButton(for page: WizardPage, state: WizardSystemState) -> Bool {
    ...
    public func primaryButtonText(for page: WizardPage, state: WizardSystemState) async -> String {
        switch page {
        case .conflicts:
            "Resolve Conflicts"
        case .inputMonitoring:
        ...
        case .helper:
            await (WizardDependencies.helperManager?.isHelperInstalled() ?? false) ? "Manage Helper" : "Install Helper"
        case .communication:
            "Check TCP Server"
        case .service:
    ...
    public func isPrimaryButtonEnabled(
    ...
    public func calculateProgress(for state: WizardSystemState) -> Double {
        switch state {
        case .initializing:
            0.0
    ...
    public func progressDescription(for state: WizardSystemState) -> String {
        switch state {
        case .initializing:
            "Checking system..."
        case .conflictsDetected:
