...
public enum Issue: Equatable {
  ...
  public var title: String {
    switch self {
    case .permissionMissing(let app, let permission, _):
      "\(app) needs \(permission) permission"
    case .componentMissing(let name, _):
      "\(name) not installed"
    case .componentUnhealthy(let name, _):
      "\(name) unhealthy"
    case .componentVersionMismatch(let name, _):
      "\(name) version incompatible"
    case .serviceNotRunning(let name, _):
      "\(name) not running"
  ...
  public var canAutoFix: Bool {
    ...
    case .componentMissing(_, let autoFix),
      .componentUnhealthy(_, let autoFix),
      .componentVersionMismatch(_, let autoFix),
      .serviceNotRunning(_, let autoFix):
      autoFix
    case .conflict:
  ...
  public var action: String {
    switch self {
    case .permissionMissing(_, _, let action):
      action
