...
class KanataManager: ObservableObject {
    ...
    private func restartKarabinerDaemon() async {
        ...
        let killScript = "do shell script \"/usr/bin/pkill -f Karabiner-VirtualHIDDevice-Daemon\" with administrator privileges with prompt \"KeyPath needs to restart the virtual keyboard daemon.\""
    ...
    private func performStartKanata() async {
        ...
        let killScript = "do shell script \"/usr/bin/pkill -f kanata\" with administrator privileges with prompt \"KeyPath needs to stop conflicting keyboard processes.\""
    ...
    func killKarabinerGrabber() async -> Bool {
        ...
        stopTask.arguments = ["-e", "do shell script \"\(stopScript)\" with administrator privileges with prompt \"KeyPath needs to stop conflicting keyboard services.\""]
    ...
    func startKarabinerDaemon() async -> Bool {
        ...
        let adminScript = "do shell script \"\(daemonPath) > /dev/null 2>&1 &\" with administrator privileges with prompt \"KeyPath needs to start the virtual keyboard daemon.\""
    ...
    private func prepareDaemonDirectories() async {
        ...
        let createDirScript = "do shell script \"mkdir -p '\(rootOnlyPath)' && chown -R \(NSUserName()) '\(tmpPath)' && chmod -R 755 '\(tmpPath)'\" with administrator privileges with prompt \"KeyPath needs to prepare system directories for the virtual keyboard.\""
        ...
        do {
            ...
            if task.terminationStatus == 0 {
                ...
                let logDirScript = "do shell script \"mkdir -p '/var/log/karabiner' && chmod 755 '/var/log/karabiner'\" with administrator privileges with prompt \"KeyPath needs to create system log directories.\""
