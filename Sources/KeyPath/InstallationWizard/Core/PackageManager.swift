...
class PackageManager {
    ...
    func installKanataViaBrew() async -> InstallationResult {
        ...
        if existingInstallation.isInstalled {
            AppLogger.shared.log("ℹ️ [PackageManager] Kanata already installed: \(existingInstallation.description)")
            return .success
        }
        ...
        if !checkBrewFormulaAvailability(formula: "kanata") {
            AppLogger.shared.log("❌ [PackageManager] Kanata formula not found - may need 'brew update' first")
        ...
        do {
            ...
            if task.terminationStatus == 0 {
                ...
                if verificationResult.isInstalled {
                    AppLogger.shared.log("✅ [PackageManager] Installation verified: \(verificationResult.description)")
                    return .success
                } else {
                    AppLogger.shared.log("❌ [PackageManager] Installation completed but Kanata binary not detected")
                    detectCommonIssues() // Log potential issues
                    return .failure(reason: "Installation completed but binary not found - possible PATH or installation issue")
                }
            } else {
                AppLogger.shared.log("❌ [PackageManager] Homebrew installation failed with status \(task.terminationStatus)")
    ...
    func getPackageManagerInfo() -> PackageManagerInfo {
        ...
        if !homebrewAvailable {
            AppLogger.shared.log("⚠️ [PackageManager] No package manager detected - automatic installation not available")
        }
        ...
        AppLogger.shared.log("📊 [PackageManager] Package manager summary: Homebrew=\(homebrewAvailable), Kanata=\(kanataInfo.isInstalled)")
    ...
    private func detectCommonIssues() {
        ...
        for homebrewPath in homebrewPaths {
            if !pathEnv.contains(homebrewPath) {
                AppLogger.shared.log("⚠️ [PackageManager] PATH may be missing \(homebrewPath) - this could prevent detection")
            }
        }
        ...
        for dir in homebrewDirs {
            if FileManager.default.fileExists(atPath: dir), !FileManager.default.fileExists(atPath: "\(dir)/bin/brew") {
                AppLogger.shared.log("⚠️ [PackageManager] Found \(dir) but no brew executable - possible incomplete installation")
            }
        }

        // Check for Cargo installation without Kanata
        let cargoPath = "\(NSHomeDirectory())/.cargo/bin"
        if FileManager.default.fileExists(atPath: cargoPath), !FileManager.default.fileExists(atPath: "\(cargoPath)/kanata") {
            AppLogger.shared.log("ℹ️ [PackageManager] Cargo detected but no Kanata binary - user may need to install via 'cargo install kanata'")
        }

        // Check for Homebrew but no Kanata formula
        if checkHomebrewInstallation(), !checkBrewFormulaAvailability(formula: "kanata") {
            AppLogger.shared.log("⚠️ [PackageManager] Homebrew available but Kanata formula not found - may need tap update")
        }
    }
    ...
    private func analyzeInstallationFailure(output: String, exitCode: Int32) {
        AppLogger.shared.log("🔍 [PackageManager] Analyzing installation failure (exit code: \(exitCode))")
        ...
        if outputLower.contains("permission denied") || outputLower.contains("operation not permitted") {
            AppLogger.shared.log("⚠️ [PackageManager] Permission issue detected - may need to fix Homebrew permissions")
            AppLogger.shared.log("💡 [PackageManager] Suggested fix: sudo chown -R $(whoami) /opt/homebrew || /usr/local")
        }
        ...
        if outputLower.contains("network") || outputLower.contains("connection") || outputLower.contains("timeout") {
        ...
        if outputLower.contains("disk") || outputLower.contains("space") || outputLower.contains("no space left") {
        ...
        if exitCode == 1, outputLower.contains("already installed") {
            AppLogger.shared.log("ℹ️ [PackageManager] Package may already be installed but not detected in PATH")
        }
    }
...
enum InstallationMethod {
    ...
    var displayName: String {
        switch self {
        case .homebrew: return "Homebrew"
        case .manual: return "Manual Download"
        case .cargo: return "Cargo (Rust)"
        }
    }
}
