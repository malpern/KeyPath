...
class SystemRequirements {
    ...
    enum MacOSVersion: Equatable {
        ...
        var isModern: Bool {
            ...
            case .legacy, .unknown: return false
            }
        }
        ...
        var isLegacy: Bool {
            ...
            case .modern, .unknown: return false
            }
        }
    ...
    enum DriverType: Equatable {
        ...
        var displayName: String {
            switch self {
            case .driverKit: return "DriverKit VirtualHIDDevice"
            case .kernelExtension: return "Kernel Extension VirtualHIDDevice"
            case .unknown: return "Unknown Driver Type"
            }
        }
        ...
        var description: String {
            switch self {
            case .driverKit:
                return "Modern DriverKit-based virtual HID device driver for macOS 11 and later"
            case .kernelExtension:
                return "Legacy kernel extension-based virtual HID device driver for macOS 10"
            case .unknown:
                return "Unable to determine appropriate driver type for this macOS version"
            }
        }
    }
    ...
    func validateSystemCompatibility() -> ValidationResult {
        ...
        if version.majorVersion < 13 {
            issues.append("KeyPath requires macOS 13.0 or later (detected: \(macosVersion.versionString))")
            isCompatible = false
        ...
        AppLogger.shared.log("🔍 [SystemRequirements] Compatibility check complete: \(isCompatible ? "✅ Compatible" : "❌ Not Compatible")")
        if !issues.isEmpty {
