...
enum BlessDiagnostics {
    static func run() -> BlessDiagnosticsReport {
        ...
        if lc.status == 0 {
            ...
            launchState = output.components(separatedBy: "\n").first(where: { $0.contains("state =") })?.trimmingCharacters(in: .whitespaces)
            launchPID = output.components(separatedBy: "\n").first(where: { $0.contains("pid =") })?.trimmingCharacters(in: .whitespaces)
            launchProgram = output.components(separatedBy: "\n").first(where: { $0.contains("program =") || $0.contains("programPath =") })?.trimmingCharacters(in: .whitespaces)
            launchLastExit = output.components(separatedBy: "\n").first(where: { $0.localizedCaseInsensitiveContains("last exit status") })?.trimmingCharacters(in: .whitespaces)
        } else {
    ...
    private static func runCmd(_ launchPath: String, _ arguments: [String]) -> (status: Int32, out: String, err: String) {
        let p = Process()
