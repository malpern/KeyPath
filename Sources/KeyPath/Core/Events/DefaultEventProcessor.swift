...
extension DefaultEventProcessor: KeyboardEventProcessing {
    ...
    private func transformKeyboardEvent(
        ...
        if keyCode == 122 { // F1 key
            AppLogger.shared.log("🔧 [DefaultEventProcessor] F1 key pressed - could trigger special action")
        }
...
public extension DefaultEventProcessor {
    /// Create a default event processor for standard KeyPath usage
    static func standard() -> DefaultEventProcessor {
    ...
    static func debugging() -> DefaultEventProcessor {
        DefaultEventProcessor(
            configuration: Configuration(
                suppressEvents: false,
                logEvents: true,
    ...
    static func suppressing() -> DefaultEventProcessor {
        DefaultEventProcessor(
            configuration: Configuration(
                suppressEvents: true,
                logEvents: false,
