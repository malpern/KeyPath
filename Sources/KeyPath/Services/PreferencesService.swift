...
final class PreferencesService {
    ...
    var udpServerEnabled: Bool {
        didSet {
            ...
            AppLogger.shared.log("🔧 [PreferencesService] UDP server enabled: \(udpServerEnabled)")
        }
    }
        ...
        didSet {
            ...
            if !isValidPort(udpServerPort) {
                AppLogger.shared.log(
                    "❌ [PreferencesService] Invalid UDP port \(udpServerPort), reverting to \(oldValue)")
                udpServerPort = oldValue
            } else {
                UserDefaults.standard.set(udpServerPort, forKey: Keys.udpServerPort)
                AppLogger.shared.log("🔧 [PreferencesService] UDP server port: \(udpServerPort)")
            }
        }
    }
            ...
            Task { @MainActor in
                do {
                    if udpAuthToken.isEmpty {
                        try await UDPAuthTokenManager.shared.clearToken()
                    } else {
                        try await UDPAuthTokenManager.shared.setToken(udpAuthToken)
                    }
                    // Remove from UserDefaults for security (migration)
                    UserDefaults.standard.removeObject(forKey: Keys.udpAuthToken)
                    AppLogger.shared.log("🔧 [PreferencesService] UDP auth token updated via TokenManager")
                } catch {
                    AppLogger.shared.log("❌ [PreferencesService] Failed to update token via TokenManager: \(error)")
                }
            }
        }
    }
    ...
    var udpSessionTimeout: Int {
        didSet {
            if udpSessionTimeout < 300 || udpSessionTimeout > 86400 { // 5 minutes to 24 hours
                AppLogger.shared.log(
                    "❌ [PreferencesService] Invalid UDP session timeout \(udpSessionTimeout), reverting to \(oldValue)")
                udpSessionTimeout = oldValue
            } else {
                UserDefaults.standard.set(udpSessionTimeout, forKey: Keys.udpSessionTimeout)
                AppLogger.shared.log("🔧 [PreferencesService] UDP session timeout: \(udpSessionTimeout) seconds")
            }
        }
    }
    ...
    init() {
        ...
        // Initialize token synchronously first, then async load
        ...
        // Load token from UDPAuthTokenManager (will handle migration internally)
        Task { @MainActor in
            let loadedToken = await UDPAuthTokenManager.shared.currentToken() ?? Defaults.udpAuthToken
            udpAuthToken = loadedToken
            ...
            if let legacyToken = legacyToken, !legacyToken.isEmpty {
                do {
                    try await UDPAuthTokenManager.shared.setToken(legacyToken)
                    UserDefaults.standard.removeObject(forKey: Keys.udpAuthToken)
                    udpAuthToken = legacyToken
                    AppLogger.shared.log("🔄 [PreferencesService] Migrated legacy UDP token")
                } catch {
                    AppLogger.shared.log("⚠️ [PreferencesService] Failed to migrate legacy token: \(error)")
                }
            }
        }
    }
    ...
    func resetUDPSettings() {
        ...
        udpSessionTimeout = Defaults.udpSessionTimeout
        AppLogger.shared.log("🔧 [PreferencesService] UDP settings reset to defaults")
    }
...
extension PreferencesService {
    ...
    /// Build UDP launch arguments with consistent precedence
    nonisolated static func buildUDPArguments(port: Int, sessionTimeout: Int, defaultTimeout: Int) -> [String] {
        var args = ["--udp-port", "\(port)"]

        // Token precedence: Cache → Keychain → Shared file → No auth
        if let cachedToken = UDPAuthTokenCache.value, !cachedToken.isEmpty {
            args.append(contentsOf: ["--udp-auth-token", cachedToken])
        } else {
            // Try Keychain
            let keychainToken: String?
            do {
                keychainToken = try KeychainService.shared.retrieveUDPToken()
            } catch {
                keychainToken = nil
            }

            if let token = keychainToken, !token.isEmpty {
                args.append(contentsOf: ["--udp-auth-token", token])
            } else if let fileToken = CommunicationSnapshot.readSharedUDPToken(), !fileToken.isEmpty {
                args.append(contentsOf: ["--udp-auth-token", fileToken])
            } else {
                // Phase 1: No auth fallback
