...
final class PreferencesService: @unchecked Sendable {
    ...
    var tcpAuthToken: String {
        get {
            ...
            AppLogger.shared.log("🔧 [PreferencesService] Reading TCP token from shared file: \(token.isEmpty ? "EMPTY" : "[REDACTED]")")
            return token
        }
        set {
            ...
            if newValue.isEmpty {
                ...
                AppLogger.shared.log("🔧 [PreferencesService] TCP auth token cleared")
            } else {
                // Set token - write to shared file
                let success = CommunicationSnapshot.writeSharedTCPToken(newValue)
                if success {
                    AppLogger.shared.log("🔧 [PreferencesService] TCP auth token updated in shared file")
                } else {
    ...
    var tcpSessionTimeout: Int {
        didSet {
            if tcpSessionTimeout < 300 || tcpSessionTimeout > 86400 { // 5 minutes to 24 hours
                AppLogger.shared.log(
                    "❌ [PreferencesService] Invalid TCP session timeout \(tcpSessionTimeout), reverting to \(oldValue)")
                tcpSessionTimeout = oldValue
            } else {
                UserDefaults.standard.set(tcpSessionTimeout, forKey: Keys.tcpSessionTimeout)
                AppLogger.shared.log("🔧 [PreferencesService] TCP session timeout: \(tcpSessionTimeout) seconds")
            }
        }
    }
    ...
    init() {
        ...
        AppLogger.shared.log(
            "🔧 [PreferencesService] Initialized - Protocol: \(communicationProtocol.rawValue), TCP enabled: \(tcpServerEnabled)"
        )
        ...
        if let legacyToken, !legacyToken.isEmpty {
            ...
            if success {
                UserDefaults.standard.removeObject(forKey: Keys.tcpAuthToken)
                AppLogger.shared.log("🔄 [PreferencesService] Migrated legacy TCP token to shared file")
            } else {
                AppLogger.shared.log("⚠️ [PreferencesService] Failed to migrate legacy token to shared file")
            }
        }
    }
...
extension PreferencesService {
    ...
    nonisolated static func buildTCPArguments(port: Int, sessionTimeout: Int, defaultTimeout: Int) -> [String] {
        ...
        if token.isEmpty {
            AppLogger.shared.log("❌ [TCP Launch] Failed to ensure TCP token; refusing to start without auth")
            // Still return args; caller should decide to abort launch if desired
        } else {
            AppLogger.shared.log("🔐 [TCP Launch] Auth token ensured; using environment variable")
        }
    ...
    var communicationEnvironmentVariables: [String: String] {
        ...
        AppLogger.shared.log("❌ [PreferencesService] Missing TCP token; not exposing TCP environment")
        return [:]
    }
}
...
struct CommunicationSnapshot: Sendable {
    ...
    nonisolated static func writeSharedTCPToken(_ token: String) -> Bool {
        ...
        do {
            ...
            AppLogger.shared.log("❌ [TCP Auth] Failed to write shared token: \(error)")
            return false
        }
    }
    ...
    nonisolated static func ensureSharedTCPToken() -> String {
        ...
        if writeSharedTCPToken(newToken) {
            AppLogger.shared.log("🔐 [TCP Auth] Generated new shared token")
            ...
            Task { @MainActor in
                do {
                    ...
                    AppLogger.shared.log("⚠️ [TCP Auth] Failed to backup token in Keychain: \(error)")
                }
            }

            return newToken
        } else {
            AppLogger.shared.log("❌ [TCP Auth] Failed to write shared token, using generated token")
            return newToken
        }
    }
