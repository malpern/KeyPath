...
actor ReloadSafetyMonitor {
    ...
    func checkReloadSafety(currentPID _: Int?) -> SafetyCheck {
        ...
        if let crashTime = crashLoopDetectedAt {
            ...
            if timeSinceCrash < crashLoopBackoffSeconds {
                ...
                AppLogger.shared.warn("⛔️ [ReloadSafety] In crash loop backoff - \(Int(remaining))s remaining")
                return .unsafe(reason: "Crash loop detected - backing off for \(Int(remaining))s")
            } else {
                // Backoff expired - clear crash loop state
                AppLogger.shared.log("✅ [ReloadSafety] Crash loop backoff expired - resuming normal operation")
        ...
        if let lastReload = lastReloadTime {
            ...
            if timeSinceLastReload < reloadCooldownSeconds {
                ...
                AppLogger.shared.debug("⏱️ [ReloadSafety] Reload cooldown active - \(String(format: "%.1f", remaining))s remaining")
        ...
        if recentRestarts >= crashLoopThreshold {
            AppLogger.shared.error("🚨 [ReloadSafety] Crash loop detected - \(recentRestarts) restarts in last \(Int(trackingWindowSeconds))s")
            crashLoopDetectedAt = now
            return .unsafe(reason: "Crash loop detected - \(recentRestarts) restarts in \(Int(trackingWindowSeconds))s")
        }
    ...
    func recordServiceRestart(pid: Int) {
        ...
        if let lastReload = lastReloadTime {
            ...
            if timeSinceReload < 60.0 { // Within 60 seconds
                AppLogger.shared.warn("⚠️ [ReloadSafety] Service restarted \(Int(timeSinceReload))s after reload - potential crash")

                // Check if this is part of a crash loop pattern
                let recentRestarts = countRecentRestarts(within: trackingWindowSeconds)
                if recentRestarts >= crashLoopThreshold {
                    AppLogger.shared.error("🚨 [ReloadSafety] Crash loop detected! \(recentRestarts) restarts in \(Int(trackingWindowSeconds))s")
                    crashLoopDetectedAt = now
                }
            }
        }
    ...
    func getStatus() -> SafetyStatus {
        let now = Date()
        let recentRestarts = countRecentRestarts(within: trackingWindowSeconds)
        let recentReloads = reloadHistory.filter { now.timeIntervalSince($0.timestamp) < trackingWindowSeconds }.count
        ...
        if let crashTime = crashLoopDetectedAt {
            ...
            backoffRemaining = elapsed < crashLoopBackoffSeconds ? (crashLoopBackoffSeconds - elapsed) : nil
        } else {
