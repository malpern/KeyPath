...
final class ProcessManager: ProcessManaging, @unchecked Sendable {
    ...
    func startService(configPath: String, arguments: [String]) async -> Bool {
        ...
        if success {
            ...
            if let pid = serviceStatus.pid {
                ...
                AppLogger.shared.log("✅ [ProcessManager] Successfully started Kanata LaunchDaemon service (PID: \(pid))")
                return true
            } else {
                AppLogger.shared.log("⚠️ [ProcessManager] Service started but no PID found - may still be initializing")
                return false
            }
        } else {
    ...
    func resolveConflicts() async {
        ...
        if !allProcesses.isEmpty {
            AppLogger.shared.log("⚠️ [ProcessManager] Found \(allProcesses.count) existing Kanata processes")
            
            for processInfo in allProcesses {
                AppLogger.shared.log("⚠️ [ProcessManager] Process PID \(processInfo.pid): \(processInfo.command)")
            }
    ...
    func verifyNoConflicts() async {
        ...
        AppLogger.shared.log("🔍 [ProcessManager] Process status: \(managedProcesses.count) managed, \(conflictProcesses.count) conflicts")
        
        // Show managed processes (should be our LaunchDaemon)
        for processInfo in managedProcesses {
            AppLogger.shared.log("✅ [ProcessManager] Managed LaunchDaemon process: PID \(processInfo.pid)")
        }
        
        // Show any conflicting processes (these are the problem)
        for processInfo in conflictProcesses {
            AppLogger.shared.log("⚠️ [ProcessManager] Conflicting process: PID \(processInfo.pid) - \(processInfo.command)")
        }
        
        if conflictProcesses.isEmpty {
            AppLogger.shared.log("✅ [ProcessManager] Clean single-process architecture confirmed - no conflicts")
        } else {
            AppLogger.shared.log("⚠️ [ProcessManager] WARNING: \(conflictProcesses.count) conflicting processes detected!")
        }
    }
