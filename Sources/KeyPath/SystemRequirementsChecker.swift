...
class SystemRequirementsChecker {
    ...
    enum SystemRequirement: String, CaseIterable {
        ...
        var description: String {
            switch self {
            case .kanataExecution:
                return "Keyboard remapping engine executable"
            case .accessibilityPermissions:
                return "Required for capturing and remapping keyboard input"
            case .inputMonitoringPermissions:
                return "Required for monitoring key press events"
            case .launchDaemonDirectory:
                return "System service configuration directory"
            case .configDirectory:
                return "KeyPath configuration storage"
            case .logDirectory:
                return "Application log storage"
            case .systemVersion:
                return "Minimum macOS 13.0 required"
            case .architecture:
    ...
    func checkRequirement(_ requirement: SystemRequirement) async -> RequirementCheckResult {
        switch requirement {
        case .kanataExecution:
            return await checkKanataExecutable()
        case .accessibilityPermissions:
            return checkAccessibilityPermissions()
        case .inputMonitoringPermissions:
            return checkInputMonitoringPermissions()
        case .launchDaemonDirectory:
            return checkLaunchDaemonDirectory()
        case .configDirectory:
            return checkConfigDirectory()
        case .logDirectory:
            return checkLogDirectory()
        case .systemVersion:
    ...
    private func generateSummary(overallStatus: SystemRequirementsReport.OverallStatus, blockingCount: Int, warningCount: Int) -> String {
        ...
        case .allSatisfied:
            return "All system requirements satisfied. KeyPath is ready to run."
        case .hasWarnings:
            return "System requirements mostly satisfied with \(warningCount) warning(s). KeyPath can run but some features may be limited."
        case .hasBlockingIssues:
            return "\(blockingCount) critical requirement(s) not met. KeyPath cannot run until these are resolved."
        case .systemError:
            return "System error encountered during requirements check. Please try again or contact support."
        }
    }
}
