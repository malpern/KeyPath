...
public struct KanataConfiguration: Sendable {
    ...
    public static func generateFromCollections(_ collections: [RuleCollection]) -> String {
        ...
        let enabledCollections = resolvedCollections.filter { $0.isEnabled }
    ...
    private static func buildCollectionBlocks(
        ...
        for collection in collections {
            ...
            if !seenActivators.contains(aliasName) {
                ...
                let definition = "(tap-hold 200 200 \(tapKey) (layer-while-held \(activator.targetLayer.kanataName)))"
        ...
        for collection in collections where collection.isEnabled {
            ...
            let entries = collection.mappings.map { mapping -> LayerEntry in
                ...
                if collection.targetLayer != .base {
                    layerOutputs[collection.targetLayer] = KanataKeyConverter.convertToKanataSequence(mapping.output)
                }
                let baseOutput: String
                if collection.targetLayer == .base {
                    baseOutput = KanataKeyConverter.convertToKanataSequence(mapping.output)
                } else {
                    baseOutput = sourceKey
                }
                return LayerEntry(
                    sourceKey: sourceKey,
