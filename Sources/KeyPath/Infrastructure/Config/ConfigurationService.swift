...
public struct KanataConfiguration: Sendable {
    ...
    public static func generateFromCollections(_ collections: [RuleCollection]) -> String {
        ...
        let enabledCollections = resolvedCollections.filter { $0.isEnabled && !$0.mappings.isEmpty }
        let disabledCollections = resolvedCollections.filter { !$0.isEnabled && !$0.mappings.isEmpty }
        let header = """
        ;; Generated by KeyPath
        ;; Rule Collections: \(resolvedCollections.filter { $0.isEnabled }.count) enabled, \(resolvedCollections.count) total
        ...
        let sourceBlock = renderDefsrcBlock(enabledCollections)
        let layerBlock = renderDeflayerBlock(enabledCollections)
        let disabledBlock = renderDisabledCollections(disabledCollections)

        return [header, sourceBlock, layerBlock, disabledBlock]
            .filter { !$0.isEmpty }
            .joined(separator: "\n")
    }
    ...
    private static func renderEnabledSourceSection(_ collection: RuleCollection) -> String {
        var lines = metadataLines(for: collection, indent: "  ", status: "enabled")
        ...
        lines.append(contentsOf: sourceKeys)
        return lines.joined(separator: "\n")
    }

    private static func renderEnabledLayerSection(_ collection: RuleCollection) -> String {
        var lines = metadataLines(for: collection, indent: "  ", status: "enabled")
        if collection.mappings.isEmpty {
            lines.append("  ;; (no mappings)")
            return lines.joined(separator: "\n")
        ...
        let layerKeys = collection.mappings.map { "  \(KanataKeyConverter.convertToKanataSequence($0.output))" }
        lines.append(contentsOf: layerKeys)
        return lines.joined(separator: "\n")
    }
    ...
    private static func renderDisabledSection(_ collection: RuleCollection) -> String {
        ...
        let sourceKeys = collection.mappings.map { KanataKeyConverter.convertToKanataKey($0.input) }.joined(separator: " ")
        let layerKeys = collection.mappings.map { KanataKeyConverter.convertToKanataSequence($0.output) }.joined(separator: " ")
    ...
    private static func metadataLines(for collection: RuleCollection, indent: String, status: String) -> [String] {
        [
...
public final class ConfigurationService: FileConfigurationProviding {
    ...
    private func validateConfigurationInTestMode(_ config: String) -> (isValid: Bool, errors: [String]) {
