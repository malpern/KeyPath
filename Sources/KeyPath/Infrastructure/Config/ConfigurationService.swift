...
public struct KanataConfiguration: Sendable {
    ...
    public static func generateFromCollections(_ collections: [RuleCollection]) -> String {
        ...
        let body = resolvedCollections.map { renderCollectionSection($0) }.joined(separator: "\n")
        return header + "\n" + body
    }

    private static let defaultEmptyConfig = generateFromCollections(defaultSystemCollections)

    private static func renderCollectionSection(_ collection: RuleCollection) -> String {
        ...
        let metadata = """
        ;; === Collection: \(collection.name) (\(collection.isEnabled ? "enabled" : "disabled")) ===
        ;; UUID: \(collection.id.uuidString)
