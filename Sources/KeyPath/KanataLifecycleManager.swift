...
class KanataLifecycleManager: ObservableObject {
    ...
    func initialize() async {
        guard stateMachine.sendEvent(.initialize) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot initialize from current state: \(currentState.rawValue)")
            return
        }
    ...
    func checkRequirements() async {
        guard stateMachine.sendEvent(.checkRequirements) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot check requirements from current state: \(currentState.rawValue)")
            return
        }
        ...
        do {
            ...
            if kanataPath.isEmpty {
                ...
                _ = stateMachine.sendEvent(.requirementsFailed, context: ["reason": "Kanata executable not found"])
                return
            }
            ...
            if !hasAccessibility {
                ...
                _ = stateMachine.sendEvent(.requirementsFailed, context: ["reason": "Accessibility permissions required"])
                return
            }
    ...
    func startInstallation() async {
        guard stateMachine.sendEvent(.startInstallation) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot start installation from current state: \(currentState.rawValue)")
            return
        }
        ...
        do {
            ...
            if success {
                ...
                _ = stateMachine.sendEvent(.installationFailed, context: ["reason": "Installation process failed"])
            }
    ...
    func startKanata() async {
        guard stateMachine.sendEvent(.startKanata) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot start Kanata from current state: \(currentState.rawValue)")
            return
        }
        ...
        do {
            ...
            if isActuallyRunning {
                ...
                _ = stateMachine.sendEvent(.kanataFailed, context: ["reason": "Process verification failed"])
            }
    ...
    func stopKanata() async {
        guard stateMachine.sendEvent(.stopKanata) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot stop Kanata from current state: \(currentState.rawValue)")
            return
        }
    ...
    func restartKanata() async {
        guard stateMachine.sendEvent(.restartKanata) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot restart Kanata from current state: \(currentState.rawValue)")
            return
        }
    ...
    func applyConfiguration(input: String, output: String) async {
        guard stateMachine.sendEvent(.configurationChanged, context: ["input": input, "output": output]) else {
            AppLogger.shared.log("❌ [LifecycleManager] Cannot apply configuration from current state: \(currentState.rawValue)")
            return
        }
        ...
        do {
            ...
            _ = stateMachine.sendEvent(.configurationFailed, context: ["error": error.localizedDescription])
        }
    }
