    ...
    executePrivilegedOperation(
      ...
      operation: {
        ...
        if uid != 0 {
          ...
          _ = Self.run(
            "/bin/launchctl", ["bootout", "gui/\(uid)", "org.pqrs.service.agent.karabiner_grabber"])  // ignore status
        }
        _ = Self.run(
          "/bin/launchctl", ["disable", "system/org.pqrs.service.daemon.karabiner_grabber"])  // ignore
        _ = Self.run(
          "/bin/launchctl", ["bootout", "system", "org.pqrs.service.daemon.karabiner_grabber"])  // ignore
