...
The challenge is avoiding misfires during fast typing. KeyPath uses split-hand detection (same-hand = letter, cross-hand = modifier) and per-finger timing to make it reliable. Read the full [Shortcuts Without Reaching guide](help:home-row-mods) for details.
