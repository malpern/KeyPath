![Setting Up KeyPath](header-installation.png)
...
<!-- screenshot: id="install-auth-prompt" method="manual" view="macOS-auth-dialog" state="helper-install-prompt" -->
...
<!-- screenshot: id="install-accessibility-settings" method="peekaboo" view="System Settings" state="privacy:accessibility,keypath:listed" -->
...
<!-- screenshot: id="install-add-kanata-binary" method="peekaboo" view="System Settings" state="privacy:accessibility,file-picker:open,path:/Library/KeyPath/bin/kanata" -->
...
**Why:** Input Monitoring lets KeyPath **read** which keys you press. This is the counterpart to Accessibility — together they form the read/write pair that makes remapping work. For a deeper look at why both are needed, see [Privacy & Permissions](help:privacy).
...
<!-- screenshot: id="install-input-monitoring-done" method="peekaboo" view="System Settings" state="privacy:input-monitoring,keypath:on,kanata:on" -->
...
- [Keyboard Concepts](help:concepts) — Layers, tap-hold, and the ideas behind modern keyboard customization
- [What You Can Build](help:use-cases) — Real examples: app launching, window tiling, Vim-style navigation
- [Shortcuts Without Reaching](help:home-row-mods) — Turn your home row into modifier keys so you never leave home position
