![Vim Navigation](header-vim-navigation.png)
...
Space is the default Leader key, but you can change it. See **[Choose Your Leader Key](help:leader-key)** for alternatives (Caps Lock, Tab, Backtick).
...
- **[Windows & App Shortcuts](help:window-management)** — Leader → W → window actions
- **Numpad** — Leader → ; → number entry
- **Symbol** — Leader → S → programming symbols
- **Function** — Leader → F → F-keys and media
- **[Quick Tweaks](help:simple-packs)** — Delete Enhancement, Mission Control
...
The base Vim Navigation pack uses macOS standard shortcuts. For word motions (⌥← and ⌥→), enable the **[Neovim in the Terminal](help:neovim-terminal)** collection which adds W and B keys.

---

## Next Steps

- **[Choose Your Leader Key](help:leader-key)** — Switch from Space to Caps Lock, Tab, or Backtick
- **[Shortcuts Without Reaching](help:home-row-mods)** — Add modifiers to your home row for Shift-selection while navigating
- **[Neovim in the Terminal](help:neovim-terminal)** — Extended Vim motions for terminal power users
- **[Keyboard Concepts](help:concepts)** — Background on layers, momentary activation, and tap-hold
