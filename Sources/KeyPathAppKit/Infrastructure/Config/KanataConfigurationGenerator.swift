...
public struct KanataConfiguration: Sendable {
    ...
    public static func generateFromCollections(
        ...
        // Kanata requires `concurrent-tap-hold yes` whenever defchordsv2 is
        // emitted — otherwise the validator refuses the config. Any enabled
        // collection with space-separated chord inputs produces chordMappings.
        if !chordMappings.isEmpty {
            defcfgLines.append("  concurrent-tap-hold yes")
        }
