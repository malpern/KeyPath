...
public struct KanataConfiguration: Sendable {
    ...
    public static func generateFromCollections(_ collections: [RuleCollection]) -> String {
        ...
        var (rawBlocks, aliasDefinitions, extraLayers, chordMappings, sequenceMappings) = buildCollectionBlocks(from: enabledCollections)
        let blocks = deduplicateBlocks(rawBlocks)
        let enabledNames = enabledCollections.map(\.name).joined(separator: ", ")
        let enabledLine = enabledNames.isEmpty ? "" : "\n;; Enabled: \(enabledNames)"
        let header = """
        ;; Generated by KeyPath — https://github.com/malpern/KeyPath\(enabledLine)
        ;; SAFETY: Only mapped keys are intercepted; unmapped keys fall back to macOS defaults.
        ...
        // If no rules configured, return minimal valid config
        // Kanata requires at least defsrc and deflayer - use caps→esc as placeholder
        if blocks.isEmpty {
            return """
            \(header)
        ...
        let sequencesBlock = renderSequencesBlock(sequenceMappings, aliases: &aliasDefinitions)
        ...
        return [header, fakeKeysBlock, sequencesBlock, aliasBlock, chordsBlock, sourceBlock, baseLayerBlock, additionalLayerBlocks]
            .filter { !$0.isEmpty }
    ...
    /// Represents a sequence mapping (keys pressed in order)
    private struct SequenceMapping {
        let id: UUID // Used to generate unique alias name
        let inputKeys: String // Space-separated keys, e.g., "j k"
        let output: String // Output action, e.g., "esc"
        let description: String?
    }

    /// Render defseq block for sequence mappings (keys pressed in order, vim-style)
    /// Kanata defseq format: (defseq seq-name (key1 key2 ...))
    /// The sequence triggers an alias when the keys are pressed in order
    private static func renderSequencesBlock(_ sequenceMappings: [SequenceMapping], aliases: inout [AliasDefinition]) -> String {
        guard !sequenceMappings.isEmpty else { return "" }

        var lines = [";; === Sequence Mappings (keys pressed in order) ==="]
        lines.append("(defseq")
        for seq in sequenceMappings {
            // Generate a unique alias name for this sequence
            let aliasName = "seq-\(seq.id.uuidString.prefix(8).lowercased())"
            // Add the sequence definition
            lines.append("  \(aliasName) (\(seq.inputKeys))")
            // Add alias that defines what the sequence does
            let outputAction = seq.output.hasPrefix("(") ? seq.output : "(macro \(seq.output))"
            aliases.append(AliasDefinition(aliasName: aliasName, definition: outputAction))
        }
        lines.append(")")
        return lines.joined(separator: "\n") + "\n"
    }
    ...
    private static func renderLayerBlock(
        ...
        let layerTitle = name == "base" ? "Base Layer" : "\(name.capitalized) Layer"
        var lines = [";; === \(layerTitle) ===", "(deflayer \(name)"]
        if blocks.isEmpty {
            lines.append("  ;; No rules configured")
        } else {
            for block in blocks {
    ...
    private static func buildCollectionBlocks(
    ...
    ) -> ([CollectionBlock], [AliasDefinition], [RuleCollectionLayer], [ChordMapping], [SequenceMapping]) {
        ...
        for collection in collections {
            ...
            if !seenActivators.contains(aliasName) {
                ...
                // Combine the expected tap-hold layer switch with fake key triggers for TCP layer notifications.
                // Ordering keeps the simple tap-hold expression intact for readability and tests while still
                // emitting enter/exit events via multi.
                let definition = "(multi (tap-hold 200 200 \(tapKey) (layer-while-held \(layerName))) (on-press-fakekey kp-layer-\(layerName)-enter tap) (on-release-fakekey kp-layer-\(layerName)-exit tap))"
        ...
        for collection in collections where collection.isEnabled {
            ...
            // Separate mappings by inputType
            // Backward compatibility: treat multi-key inputs with inputType == .single as chords
            // (legacy mappings didn't have inputType, so they defaulted to .single)
            let regularMappings = effectiveMappings.filter {
                $0.inputType == .single && !$0.input.contains(" ")
            }
            let chordInputMappings = effectiveMappings.filter {
                $0.inputType == .chord || ($0.inputType == .single && $0.input.contains(" "))
            }
            let sequenceInputMappings = effectiveMappings.filter { $0.inputType == .sequence }
            ...
            // Add sequence mappings to the separate sequence list
            for mapping in sequenceInputMappings {
                let inputKeys = mapping.input.split(separator: " ").map { KanataKeyConverter.convertToKanataKey(String($0)) }.joined(separator: " ")
                let output = KanataKeyConverter.convertToKanataSequence(mapping.output)
                sequenceMappings.append(SequenceMapping(id: mapping.id, inputKeys: inputKeys, output: output, description: mapping.description))
            }

            if regularMappings.isEmpty {
                if !chordInputMappings.isEmpty || !sequenceInputMappings.isEmpty {
                    // Collection has only chord/sequence mappings - add metadata comment
                    if !chordInputMappings.isEmpty {
                        metadata.append("  ;; (chord mappings in defchordsv2 block)")
                    }
                    if !sequenceInputMappings.isEmpty {
                        metadata.append("  ;; (sequence mappings in defseq block)")
                    }
                } else {
            ...
            var entries = regularMappings.map { mapping -> LayerEntry in
                ...
                let convertedOutput = KanataKeyConverter.convertToKanataSequence(mapping.output)
                ...
                if mapping.behavior != nil {
                    ...
                    layerOutput = "@\(aliasName)"
                } else if mapping.output.hasPrefix("("), convertedOutput.hasPrefix("(") {
        ...
        return (activatorBlocks + blocks, aliasDefinitions, additionalLayers, chordMappings, sequenceMappings)
    }
    ...
    private enum KeyboardGridFormatter {
        ...
        static func renderDefsrc(_ block: CollectionBlock) -> [String] {
            ...
            // defsrc must use sparse rendering - underscore (_) is NOT valid in defsrc
            let body = renderGridLinesSparse(sorted) { $0.sourceKey }
        ...
        /// Used for defsrc where underscore (_) is NOT valid.
        private static func renderGridLinesSparse(
            _ entries: [LayerEntry],
            valueProvider: (LayerEntry) -> String
        ) -> [String] {
            var rows: [[String]] = []
            ...
            for layoutRow in layoutRows {
                // Only include keys that are actually mapped (no _ placeholders)
                let tokens = layoutRow.compactMap { key -> String? in
                    guard let entry = entryMap[key] else { return nil }
                    return valueProvider(entry)
                }
        ...
        private static func padRow(_ tokens: [String]) -> String {
            // Pad each token to consistent width for alignment
            let width = max(tokens.map(\.count).max() ?? 0, 4) // minimum 4 chars for readability
...
public final class ConfigurationService: FileConfigurationProviding {
    ...
    public func saveConfiguration(
        ...
        let collectionsToInclude: [RuleCollection] = ruleCollections
