  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 16) {
      ...
      VStack(alignment: .leading, spacing: 8) {
        ...
        Text(
          "KeyPath no longer uses a headless LaunchAgent. The app is UI-only; if a legacy agent was previously enabled, you can disable it below."
        )
        .font(.caption)
        ...
        HStack(spacing: 8) {
          ...
          .disabled(
            isCheckingStatus || !(LaunchAgentManager.isInstalled() && LaunchAgentManager.isLoaded())
          )
