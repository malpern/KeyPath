    ...
    private var outputTypeSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            InspectorButton(
                icon: "app.badge",
                title: "Launch App",
                subtitle: viewModel.selectedApp?.name ?? "Pick an application",
            ...
            InspectorButton(
                icon: "link",
                title: "Open URL",
                subtitle: viewModel.selectedURL ?? "Enter a web address",
    ...
    private var systemActionsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                ForEach(SystemActionInfo.allActions.prefix(8)) { action in
                    SystemActionButton(
                        action: action,
            ...
            // Show more button if there are more actions
            if SystemActionInfo.allActions.count > 8 {
                DisclosureGroup("More Actions") {
                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                        ForEach(SystemActionInfo.allActions.dropFirst(8)) { action in
                            SystemActionButton(
                                action: action,
                                isSelected: viewModel.selectedSystemAction?.id == action.id
                            ) {
