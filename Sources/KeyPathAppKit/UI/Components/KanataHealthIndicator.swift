...
struct KanataHealthIndicator: View {
    ...
    private var borderColor: Color {
        ...
        case .degraded: Color.orange.opacity(0.3)
        case .critical: Color.red.opacity(0.3)
        }
    }

    private var shadowColor: Color {
        switch errorMonitor.healthStatus {
        case .healthy: Color.green.opacity(0.2)
        case .degraded: Color.orange.opacity(0.2)
        case .critical: Color.red.opacity(0.2)
        }
    }
    ...
    private var usesSolidChip: Bool {
        switch errorMonitor.healthStatus {
        case .healthy: true
        case .degraded, .critical: true
        }
    }

    private var tooltip: String {
        let base = switch errorMonitor.healthStatus {
        case .healthy:
            "Kanata Engine: Healthy"
        case let .degraded(reason):
            "Kanata Engine: \(reason)"
        case let .critical(reason):
            "Kanata Engine: Critical - \(reason)"
        }
    ...
    private var accessibilityLabel: String {
        switch errorMonitor.healthStatus {
        case .healthy: "Kanata engine healthy"
        case .degraded: "Kanata engine has warnings"
        case .critical: "Kanata engine has critical errors"
        }
    }
...
@MainActor
private func openSettings(tab _: SettingsTab) {
