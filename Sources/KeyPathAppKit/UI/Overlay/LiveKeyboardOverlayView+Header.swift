...
private struct NativeDragView: NSViewRepresentable {
    ...
    func makeNSView(context _: Context) -> NativeDragNSView {
        let view = NativeDragNSView()
        view.onDragChange = onDragChange
        return view
    }

    func updateNSView(_ nsView: NativeDragNSView, context _: Context) {
...
struct OverlayDragHeader: View {
    ...
    @State var isLayerPickerOpen = false
    @State var availableLayers: [String] = ["base", "nav"]
