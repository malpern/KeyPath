...
public actor AnthropicConfigRepairService: ConfigRepairService {
    ...
    public func repairConfig(config: String, errors: [String], mappings: [KeyMapping]) async throws -> String {
        // The bundled engine is compiled without kanata's `cmd` feature (#879), so
        // (cmd ...) actions fail at parse — tell the model not to produce them.
        // process-unmapped-keys yes matches KanataDefcfg.repairFallback.
        let defcfgInstruction =
