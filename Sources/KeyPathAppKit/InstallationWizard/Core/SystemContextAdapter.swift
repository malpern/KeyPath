...
struct SystemContextAdapter {
    ...
    private static func adaptSystemState(_ context: SystemContext) -> WizardSystemState {
        ...
        if !missingPerms.isEmpty {
            AppLogger.shared.log("📊 [SystemContextAdapter] Decision: MISSING PERMISSIONS (\(missingPerms.count) missing)")
        ...
        if !missingComponents.isEmpty {
            AppLogger.shared.log("📊 [SystemContextAdapter] Decision: MISSING COMPONENTS (\(missingComponents.count) missing)")
    ...
    private static func adaptIssues(_ context: SystemContext) -> [WizardIssue] {
        ...
        if !context.helper.isReady {
            issues.append(WizardIssue(
                identifier: .component(context.helper.isInstalled ? .privilegedHelperUnhealthy : .privilegedHelper),
                severity: .error,
                category: .backgroundServices,
                title: "Privileged Helper Not Ready",
                description: "Privileged helper is not installed or not working",
                autoFixAction: context.helper.isInstalled ? .reinstallPrivilegedHelper : .installPrivilegedHelper,
                userAction: nil
            ))
        }
    ...
    private static func determineAutoFixActions(_ context: SystemContext) -> [AutoFixAction] {
        ...
        if !context.helper.isReady {
            actions.append(context.helper.isInstalled ? .reinstallPrivilegedHelper : .installPrivilegedHelper)
        }
