...
public enum URLMappingFormatter {
    private static let unreserved = CharacterSet(charactersIn: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~")

    /// Encode a URL so it is safe inside `open:` push-msg payloads.
    public static func encodeForPushMessage(_ url: String) -> String {
    ...
    public static func decodeFromPushMessage(_ url: String) -> String {
        url.removingPercentEncoding ?? url
    }

    /// Format a URL for display (prefers host, strips scheme/path when possible).
    public static func displayDomain(for url: String) -> String {
