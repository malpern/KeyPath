...
struct LauncherService {
    ...
    func shouldUseInProcessRuntime() -> Bool {
        environmentFlag(LauncherConstants.inProcessRuntimeEnvKey)
    }
    ...
    func runExperimentalPassthruRuntimeProbe(
        ...
        let shouldInject = environmentFlag(LauncherConstants.passthruInjectEnvKey)
        let shouldForward = environmentFlag(LauncherConstants.passthruForwardEnvKey)
        let shouldCapture = environmentFlag(LauncherConstants.passthruCaptureEnvKey)
        ...
        if shouldPersist {
            ...
            while true {
                ...
                usleep(LauncherConstants.passthruPollIntervalMillis * 1000)
            }
        }

        let shouldPoll = shouldForward || shouldInject
        let pollDurationMillis = environmentUInt64(
            LauncherConstants.passthruPollDurationEnvKey,
            defaultValue: LauncherConstants.defaultPassthruPollDurationMillis
        )
        ...
        while forwardedCount < LauncherConstants.passthruForwardLimit {
        ...
        logLine("Experimental passthru runtime hit forwarding limit of \(LauncherConstants.passthruForwardLimit) event(s)")
    }
        ...
        let eventTap = CGEvent.tapCreate(
            ...
            callback: { _, _, event, refcon in
                ...
                guard let inputEvent = ExperimentalHostPassthruInputMapper.eventForKeyCode(keyCode, isKeyDown: isKeyDown) else {
                    state.service.logLine("Experimental passthru capture ignoring unsupported mac keyCode=\(keyCode)")
                    return Unmanaged.passUnretained(event)
                }
                ...
                case let .failure(error):
                    state.service.logLine(error.logSummary)
                }
